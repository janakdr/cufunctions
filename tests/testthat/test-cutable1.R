...
test_that("cutable1(tcchange, Diet) summary table matches golden", {
  out <- capture.output(with(NEJM, cutable1(tcchange, Diet, plot = "no")))
...
test_that("cutable1(NEJM) summary table matches golden", {
  out <- capture.output(with(NEJM, cutable1(NEJM, plot = "no")))
...
test_that("cutable1(NEJM, Diet, brief=T) matches golden", {
  out <- capture.output(with(NEJM, cutable1(NEJM, Diet, brief = TRUE, plot = "no")))
